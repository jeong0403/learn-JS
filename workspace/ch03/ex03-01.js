...
const Score = function(kor, eng){
  ...
  // 생성자 함수 내에 this로 메서드 정의
  // s1과 s2처럼 sum은 요청할 때마다 계속 새롭게 생성됨
  // sum은 계속 생성되니까 메모리 측면에서 낭비
  // sum도 avg처럼 생성자 함수 외부에 만드는게 더 효율적인 코드임
  this.sum = function(){
...
// 생성자 함수 외부에서 생성자 함수의 prototype의 메서드 정의
// avg는 모든 인스턴스가 공유됨
...
console.log(s1.hasOwnProperty('sum')); // true hasOwnProperty -> 속성을 직접 가지고 있냐?
console.log(s1.hasOwnProperty('avg')); // false -> prototype에 속성이 들어가 있기 때문에 false