...
// 전체 수강생 수를 저장할 변수
let sum = 0;
// flag는 true/false 쓸 때 범용적으로 사용하는 이름
// let flag = false; // 전역 변수 작성. global(전역) scope(유효 범위)
let isInStudent = false; // 로 의미있는 변수로 작성해도 됨
...
for (let i = 0; i < lionStudents.length; i++) {
  ...
  // 바로핑이라는 수강생이 있는가?
  // 배열 속으로 들어가서 내부 포문 각 줄에서 한 번 더 돌아서 확인 함
