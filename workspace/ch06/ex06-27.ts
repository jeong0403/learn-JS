...
(()=>{
  ...
  interface User {
    name: string;
    age: number;
    admin: false; // admin이 true/false 여부로 User 타입인지 아닌지 확인할 수 없음 (boolean이라고 적었을 때)
  }

  // 관리자
  interface AdminUser {
    name: string;
    level: 1 | 2 | 3; // level에 따른 권한 분리
    admin: true; // admin이 true/false 여부로 AdminUser 타입인지 아닌지 확인할 수 없음 (boolean이라고 적었을 때)
  }
  ...
  // 타입 가드 구문이 복잡하거나 여러 번 재사용 해야 할 경우 타입 가드 함수를 작성
  ...
  function isGuest(user: User | AdminUser | Guest): user is Guest {
    return !('age' in user) && !('level' in user) && user.name === '게스트'; // 이게 true를 리턴하면 게스트다~
  }
